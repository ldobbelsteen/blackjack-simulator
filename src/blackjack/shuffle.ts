...
export class Shuffler {
  ...
  private getEntropy(): void {
    crypto.getRandomValues(this.entropy);
    this.index = 0;
  }

  /** Get a random Uint16 (two bytes) from the entropy */
  private randomTwoBytes(): number {
    const byte = this.entropy[this.index++];
    if (this.index === this.length) this.getEntropy();
    return byte;
  }
  ...
  shuffle<T>(arr: T[]): void {
    ...
    for (let index = length - 1, random; index > 0; index--) {
      ...
      do {
        random = this.randomTwoBytes() & mask;
      } while (random > index);
